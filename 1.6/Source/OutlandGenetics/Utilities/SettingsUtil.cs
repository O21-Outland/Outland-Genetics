...
namespace OutlandGenes
{
    public static class SettingsUtil
    {
        ...
        public static void SettingsDropdown(this Listing_Standard listing, string name, string explanation, ref OutlandGenesSettingsPage value, float width)
        {
            float curHeight = listing.CurHeight;
            Rect rect = listing.GetRect(Text.LineHeight + listing.verticalSpacing);
            Text.Font = GameFont.Small;
            ...
            if (Widgets.ButtonText(rect2, value.ToString().Replace("_", " "), true, true, true))
            {
                ...
                List<OutlandGenesSettingsPage> enumValues = Enum.GetValues(typeof(OutlandGenesSettingsPage)).Cast<OutlandGenesSettingsPage>().ToList();
                foreach (OutlandGenesSettingsPage enumValue in enumValues)
                {
                    list.Add(new FloatMenuOption(enumValue.ToString().Replace("_", " "), delegate ()
                    {
                        OutlandGenesMod.mod.currentPage = enumValue;
                    }));
                }
    ...
    public static class EnhancedListingStandard
    {
        ...
        public static void AddGeneSetting(this Listing_Standard listing, GeneDef gene, ref bool value)
        {
            float curHeight = listing.CurHeight;
            listing.Gap(EnhancedListingStandard.Gap);
            listing.CheckboxLabeled(gene.LabelCap, ref value, null);
            Rect rect = listing.GetRect(0f);
            rect.height = listing.CurHeight - curHeight;
            rect.y -= rect.height;
            if (Mouse.IsOver(rect))
